...
app.post('/save-itinerary', async (req, res) => {
    try {
        ...
        for (const day of itinerary) {
            ...
            await db.execute(`
                INSERT INTO SavedItinerary (trip_id, day_number, activity_order,user_id)
                VALUES (?, ?, ?, ?)
                ON DUPLICATE KEY UPDATE activity_order = VALUES(activity_order)
            `, [tripId, day.day_number, orderedActivities,userId]);
        }
...
app.get('/rajasthan', async (req, res) => {
    try {
        const [tripRows] = await db.execute(
            "SELECT * FROM Trips WHERE destination = 'rajasthan'"
        );
...
//         const { tripId } = req.query;  // Get trip ID from query params

//         console.log("Fetching itinerary for User ID:", userId, "Trip ID:", tripId);
...
//             "SELECT day_number, activity_order FROM SavedItinerary WHERE trip_id = ? AND user_id = ? ORDER BY day_number",
//             [tripId, userId]
//         );
...
// Route to serve the static "Start Planning" HTML page from public folder
// app.get('/start-planning', (req, res) => {
//     res.sendFile(__dirname + '/public/start-planning.html'); // Load the HTML page
// });
...
// Route for "Saved Plans" that fetches data dynamically
app.get('/saved-plans', async (req, res) => {
    try {
        if (!req.session.userId) {
        ...
        const [itineraries] = await db.execute(
            `SELECT si.*, t.trip_name, t.destination 
            FROM SavedItinerary si 
            JOIN Trips t ON si.trip_id = t.id 
            WHERE si.user_id = ? 
        ORDER BY si.trip_id, si.day_number`,
    [userId]
        );

        res.render('saved-itinerary', { itineraries }); // Render the EJS page
    } catch (err) {
        console.error("Error fetching saved plans:", err);
        res.status(500).send("Error loading saved plans");
    }
});
